...
const url = '..\Resources\data.sqlite'
const dataPromise = d3.json(url);
    console.log("Data Promise: ", dataPromise);
    Exploring-Music-Using-Spotify_API/Resources/data.sqlite
...
function hbarChart(idSelection) {
    ...
    let y_axis = idSelection.popularity
        .slice(0, 10)
        .reverse()
        .map((item) => `value${item}`);
    let text = idSelection.value_labels.slice(0, 10).reverse();
